...
- 🔧 **多骨干网络支持**: 支持 CSPDarknet、ResNet、MobileNetV3、VGG 等多种骨干网络
...
| 骨干网络 | 说明 | 参数量 | 适用场景 |
|---------|------|--------|---------|
| CSPDarknet | YOLO 原生骨干网络 | 中等 | 通用目标检测 |
| ResNet50 | 经典残差网络 | 较大 | 迁移学习、高精度需求 |
| ResNet101 | 更深的 ResNet | 大 | 高精度需求 |
| MobileNetV3 | 轻量级网络 | 小 | 移动端部署、实时检测 |
| VGG16 | 经典 VGG 网络 | 大 | 教学、研究 |
| VGG19 | 更深的 VGG | 大 | 教学、研究 |
...
model:
  ...
  backbone_pretrained: true    # 是否使用预训练权重
  width_multiple: 0.5          # 宽度缩放因子
...
```yaml
model:
  backbone_name: 'ResNet50'      # 可选: CSPDarknet, ResNet50, ResNet101, MobileNetV3, VGG16, VGG19
  backbone_pretrained: true      # 使用 ImageNet 预训练权重
```
